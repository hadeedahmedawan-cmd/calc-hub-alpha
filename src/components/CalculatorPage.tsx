...
import { ALL_CALCULATORS, slugifyCategory } from "@/lib/calculators/registry";
import { CATEGORY_DISCLAIMERS } from "@/lib/calculators/disclaimers";
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10">
      <nav aria-label="Breadcrumb" className="mb-6 text-xs text-muted-foreground">
        ...
        <Link to="/category/$slug" params={{ slug: slugifyCategory(calc.category) }} className="hover:text-foreground">
          {calc.category}
        </Link>
        <span className="mx-2">/</span>
      ...
      {calc.custom ? (
        ...
        <div className="mt-8 space-y-4">
          ...
          {CATEGORY_DISCLAIMERS[calc.category] && (
...
function CalculatorContentSection({ content }: { content: CalculatorContentData }) {
  return (
    <section className="mt-10 border-t border-border pt-8 space-y-8">
      ...
      {content.relatedGuide && (
        <div className="mt-6 rounded-lg border border-border bg-secondary/30 p-4">
          <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Related guide</p>
          <Link
            to={content.relatedGuide.href}
            className="mt-1 block text-sm font-medium text-foreground underline underline-offset-2"
          >
            {content.relatedGuide.title} →
          </Link>
        </div>
      )}
    </section>
